	...
	public partial class ThreadsTestForm : Form
	{
		...
		private void StartThreads()
		{
			...
			for (int nIx = 0; nIx < nThreads; nIx++)
			{
				...
				Thread thrdItem = new Thread(RunThread);
		...
		private void TimerStatus(Object objState)
		{
			...
			if (!bStillRunning)
			{
				...
				Invoke(actionSetState, new Object[] { numThreadCount, CTRL_STATE.CTRL_ENABLED }); // btnStartStop.Text = "&Start";
				m_lstThreads.Clear();
		...
		private Action<Control, CTRL_STATE> actionSetState = (Control ctrlItem, CTRL_STATE ctrlState) => { ctrlItem.Enabled = (ctrlState == CTRL_STATE.CTRL_ENABLED) ? true : false; };
